...
import json, base64, os, subprocess, sys, urllib.request, zipfile, threading, hashlib
...
# PASSWORD key for AES
SECRET_PASSWORD = 'MySuperSecretKeyStore2026'
CONFIG_FILE = 'license_config.json'
SERVER_APPS_JSON_URL = 'https://github.io'

def derive_key(p): return hashlib.sha256(p.encode()).digest()[:16]

def decrypt_short_serial(s):
    try:
        k = derive_key(SECRET_PASSWORD)
        cs = s.replace('-', '')
        rem = len(cs) % 8
        if rem > 0: cs += '=' * (8 - rem)
        cb = base64.b32decode(cs.encode())
        cipher = Cipher(algorithms.AES(k), modes.CBC(b'0123456789abcdef'), backend=default_backend())
        dec = cipher.decryptor()
        txt = (dec.update(cb) + dec.finalize()).decode().strip()
        return True, int(txt.split(':')[-1])
...
class AppStoreForComputer(ctk.CTk):
    ...
    def prompt_activation(self):
        ...
        if code: threading.Thread(target=self.verify_key_online, args=(code.strip(),), daemon=True).start()

    def verify_key_online(self, code):
        try:
            req = urllib.request.Request('https://github.io, headers={'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req, timeout=5) as res: data = json.loads(res.read().decode())
            if data.get('status') == 'success':
                with open(CONFIG_FILE, 'w') as f: json.dump({'serial': code, 'hwid': self.current_hwid, 'expiry_date': data.get('expiry')}, f)
                self.is_activated = True
                self.after(0, self.show_main_page)
                self.after(0, lambda: self.show_popup('Activated', 'Successfully Unlocked All Features!'))
            else: self.after(0, lambda: self.show_popup('Failed', data.get('message', 'Invalid Key')))
        except: self.after(0, lambda: self.show_popup('Error', 'Could not connect to host server!'))
