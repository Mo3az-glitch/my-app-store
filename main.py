...
class AppStoreForComputer(ctk.CTk):
    ...
    def show_app_details_page(self, app):
        ...
        back_btn = ctk.CTkButton(self.main_container, text="← Back to Store", width=120, fg_color="#333333", command=self.show_main_page)
        back_btn.pack(anchor="w", padx=30, pady=20)
        
        details_box = ctk.CTkFrame(self.main_container, fg_color="#1f1f1f", corner_radius=18)
        details_box.pack(fill="both", expand=True, padx=30, pady=10)
        
        ctk.CTkLabel(details_box, text=app["name"], font=ctk.CTkFont(size=32, weight="bold"), text_color="white").pack(anchor="w", padx=40, pady=30)
        ctk.CTkLabel(details_box, text=app["desc"], font=ctk.CTkFont(size=16), text_color="gray", wraplength=700, justify="left").pack(anchor="w", padx=40, pady=10)
        
        if self.is_activated:
            btn = ctk.CTkButton(details_box, text="Download Now (Silent Install)", size=(250, 50), fg_color="#2ec4b6", font=ctk.CTkFont(size=16, weight="bold"), command=lambda: self.download_app(app))
        else:
            btn = ctk.CTkButton(details_box, text="🔒 Premium License Required", size=(250, 50), state="disabled", fg_color="#444444")
        ...
        if code:
            ...
            if success:
                ...
                with open(CONFIG_FILE, "w") as f:
                    json.dump({"serial": code, "hwid": self.current_hwid, "expiry_date": calculated_expiry}, f)
                self.is_activated = True
                self.show_main_page()
